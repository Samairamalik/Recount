...
def test_clean_report_end_to_end_has_no_fail_and_no_rejection() -> None:
    ...
    # config wording for their metric ("to hit 1,447,714.17", "a year-low"); changelog 7's
    # alias "days for delivery" returns two: 43 / 18, 13 metric_echo_failed.
    assert len(ex.claims) == 61
    assert Counter(v.verdict for v in result.verdicts) == {"PASS": 43, "UNVERIFIABLE": 18}
    assert {v.abstain_reason.value for v in result.verdicts if v.abstain_reason} == {
        "ambiguous",
        "unsupported_claim_type",
        "schema_gap",
    }
    assert sum(v.detail.startswith("metric_echo_failed") for v in result.verdicts) == 13
